...
import { useLocation, useNavigate } from "react-router-dom";
import api from "../services/api";
import { setToken, setRememberMe, getRememberedEmail, isRememberMe } from "../utils/auth";
import sessionManager from "../utils/sessionManager";
import { jwtDecode } from "jwt-decode";
import {
  ShieldCheck,
...
export default function LoginPage() {
  ...
  const [waiting, setWaiting] = useState(false);
  const [formData, setFormData] = useState({ email: getRememberedEmail(), password: "" });
  const [rememberMe, setRememberMeState] = useState(isRememberMe());
  const [error, setError] = useState("");
  const [Password, SetPasswordvisible] = useState(false);
